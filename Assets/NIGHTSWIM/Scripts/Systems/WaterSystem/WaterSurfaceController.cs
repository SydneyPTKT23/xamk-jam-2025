...
namespace slc.NIGHTSWIM.WaterSystem
{
    public class WaterSurfaceController : MonoBehaviour
    {
        ...
        public WaterSettings m_waterSettings;
        ...
        private void Update()
        {
            WaterSettings.WaveLayer[] t_waveLayers = m_waterSettings.WaveLayers;
            NativeArray<WaterSettings.WaveLayer> t_nativeLayers = new(t_waveLayers, Allocator.TempJob);

            // Create job data for UpdateWaterHeightJob
            UpdateWaterHeightJob t_updateJob = new()
            {
                Dimension = Dimension,
                WaveLayers = t_nativeLayers,
                Time = Time.time,
